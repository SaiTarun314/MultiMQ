...
private:
    ...
    std::map<std::string, std::unique_ptr<PartitionService::Stub>> partitionStubs;       // HashMap which stores the Partition Stub for each partition
...
public:
    std::unique_ptr<PartitionService::Stub> getStub(const std::string& partitionName) { 
        return std::move(partitionStubs[partitionName]);
    }
